...
def plot_boxplot(data, cenario, variable):
    ...
    if cenario == 'monophasic_station' or cenario == 'stations_mix':
        x_data = ['20%', '40%', '60%', '80%', '100%']
    else:
        x_data = ['5.0%', '10%', '15%', '20%', '25%']
    y_data = data

    colors = ['rgba(213, 228, 207, 0.8)', 'rgba(110, 181, 205, 0.8)',
              'rgba(42, 132, 107, 0.8)', 'rgba(18, 107, 105, 0.8)', 'rgba(2, 65, 83, 0.8)']
    ...
    if variable == 'losses':
        fig.update_layout(
            title='Perdas no Sistema de Distribuição',
            yaxis_title='Índice de Perdas Elétricas (%)', xaxis_title='Níveis de Penetração de VEs',
        ...
        fig.update_layout(
            title='Carregamento dos Transformadores em Sobrecarga',
            yaxis_title='Percentual da carga nominal (%)', xaxis_title='Níveis de Penetração de VEs',
    ...
    fig.write_image(r"D:\pedro\images\fig_" + cenario + "_" + variable + ".pdf")
...
def plot_double_boxplot(data1, data2, cenario, variable):
    ...
    fig.update_layout(
        title='Níveis de tensão no Sistema de Distribuição',
        width=1000, height=750,
        yaxis=dict(autorange=True, showgrid=True, zeroline=False,
                   gridcolor='rgb(208, 206, 206)', gridwidth=1,
                   zerolinecolor='rgb(0, 0, 0)', zerolinewidth=2, ),
